...
public class TransactionManagerLocal {
	...
	private static final Logger LOGGER = LogManager
			.getLogger(TransactionManagerLocal.class.getName());
	...
	public void finalize() throws Throwable {
		...
		if (!released) {
			...
			Debug.logtnx(formattedTransactionID()
					+ " DEV_ERROR  :  TransactionManager.release()"
	...
	public int executeDML(String sql) throws SQLException {
		...
		if (commitHasBeenCalledAlready) {
			throw new SQLException(
					"DEV_ERROR : Already commited TransactionManager"
							+ " object is used without calling reset() : "
							+ whereiam);
		}
	...
	public int executePreparedDML() throws SQLException {
		...
		// Ensure the given query is a either INSERT or UPDATE or DELETE or DROP
		// TABLE (DROP T)
		// Also ensures that the qury may be a DROP query
		String iudPart = sql.trim().substring(0, 6);
		if ((!iudPart.equalsIgnoreCase("INSERT"))
				&& (!iudPart.equalsIgnoreCase("UPDATE"))
				&& (!iudPart.equalsIgnoreCase("DELETE"))
				&& (!iudPart.equalsIgnoreCase("DROP T"))) {

			throw new SQLException(
					"Not an Insert/Update/Delete/Drop query : \"" + sql
							+ "\" : " + whereiam);
		}
		// Trim sql
		sql = sql.trim();
	...
	public void commit() throws SQLException {
		...
		if (commitHasBeenCalledAlready) {
			String msg = "DEV_ERROR : Please call TransactionManager.commit()"
					+ " only once. You already have called it on the"
					+ " same object of TransactionManager. If you want to"
					+ " call commit() again then create a fresh object"
					+ " -OR- call reset() on this object first.";
		...
		if (sqlList.size() <= 1) {
			// Debug.logtnx(formattedTransactionID() +
			// " 0.o.^.o.0   This transaction" + " has only one DML. Better use"
			// + " TransactionManager.executeDMLIndividualTnx(sql)" +
			// "   0.o.^.o.0 " + whereiam);
		}
		...
		// Debug.logtnx(formattedTransactionID() +
		// ":) ...Transaction committed successfully... " + whereiam);
	...
	// public PreparedStatement prepareStatement(String sql, int scroll, int
	// conqyirency) throws SQLException {
	//
	// prstmt = null;
	...
	public String toString() {
		...
		// Make toString
		String str = "whereiam=" + whereiam + " transactionSuccessfull="
				+ transactionSuccessfull + " commitHasBeenCalledAlready="
				+ commitHasBeenCalledAlready + " transactionID="
				+ transactionID + " transactionCounter=" + transactionCounter
				+ " releasedTransactionCounter=" + releasedTransactionCounter
				+ " sqlCount=" + sqlCount + "\n" + sqls.toString();
		return str;
	}
	...
	public static int executeDMLIndividualTnx(String iudSQL, String whereIAm,
		...
		if (iudSQL == null || iudSQL.trim().equals("")) {
			throw new SQLException("DEV_ERROR : Empty sql passed : \"" + iudSQL
					+ "\" : " + whereIAm);
		}
		...
		if ((!iudPart.equalsIgnoreCase("INSERT"))
			...
			throw new SQLException(
					"Not an Insert/Update/Delete/Drop query : \"" + iudSQL
							+ "\" : " + whereIAm);
		}
		...
		try {
			...
			if (nRows < 1 && !iudPart.equalsIgnoreCase("DROP T")) {
				throw new SQLException(
						"No records updated/added/deleted in the DB : " + nRows
								+ " In : " + whereIAm);
			}
		} finally {
			try {
